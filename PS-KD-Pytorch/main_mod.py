...
def parse_args():
    ...
    ## additional setting ##
    parser.add_argument('--use_BYOT', action='store_true', help='use be your own teacher')
    parser.add_argument('--use_DML', action='store_true', help='use DML')

    # from be your own teacher
...
def main_worker(gpu, ngpus_per_node, model_dir, log_dir, args):
    ...
    ## giving information about setting ##
    if args.use_BYOT:
...
def train(all_predictions,
    ...
    for batch_idx, (inputs, targets, input_indices) in enumerate(train_loader):
        ...
        if args.PSKD:
            ...
            # student model
            # compute output
            if args.classifier_type in ["ResNetBeMyOwnTeacher18", "ResNetBeMyOwnTeacher50"] :
        ...
        if args.use_BYOT:
            ...
            if args.PSKD:
                middle1_loss = criterion_CE_pskd(middle_output1, soft_targets)
                middle1_losses.update(middle1_loss.item(), inputs.size(0))
                middle2_loss = criterion_CE_pskd(middle_output2, soft_targets)
                middle2_losses.update(middle2_loss.item(), inputs.size(0))
                middle3_loss = criterion_CE_pskd(middle_output3, soft_targets)
                middle3_losses.update(middle3_loss.item(), inputs.size(0))
            else:
                middle1_loss = criterion_CE(middle_output1, soft_targets)
                middle1_losses.update(middle1_loss.item(), inputs.size(0))
                middle2_loss = criterion_CE(middle_output2, soft_targets)
                middle2_losses.update(middle2_loss.item(), inputs.size(0))
                middle3_loss = criterion_CE(middle_output3, soft_targets)
                middle3_losses.update(middle3_loss.item(), inputs.size(0))
            ...
            middle1_prec1 = accuracy(middle_output1.data, targets, topk=(1,))
            middle1_top1.update(middle1_prec1[0], inputs.size(0))
            middle2_prec1 = accuracy(middle_output2.data, targets, topk=(1,))
            middle2_top1.update(middle2_prec1[0], inputs.size(0))
            middle3_prec1 = accuracy(middle_output3.data, targets, topk=(1,))
            middle3_top1.update(middle3_prec1[0], inputs.size(0))

            kl_loss = 0
            if args.use_DML:
                #### DML Idea Implementation ####
                temp_ls_for_DML = [middle_output1, middle_output2, middle_output3]
                for templogit in temp_ls_for_DML:
                    kl_loss += criterion_KL(F.log_softmax(outputs, dim = 1), 
                                                    F.softmax(Variable(templogit), dim=1))
                kl_loss = kl_loss / len(temp_ls_for_DML)
                ##****
        
        else:
            kl_loss = 0
            if args.PSKD:
                total_loss = criterion_CE_pskd(outputs, soft_targets)
            else:
                total_loss = criterion_CE(outputs, targets)
        
        actual_loss = total_loss + kl_loss
    
        train_losses.update(actual_loss.item(), inputs.size(0))
...
def val(criterion_CE,
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, _) in enumerate(val_loader):              
            ...
            if args.use_BYOT:
                # monitor with hard label
                losses.update(loss.item(), inputs.size(0))
                middle1_loss = criterion_CE(middle_output1, targets)
                middle1_losses.update(middle1_loss.item(), inputs.size(0))
                middle2_loss = criterion_CE(middle_output2, targets)
                middle2_losses.update(middle2_loss.item(), inputs.size(0))
                middle3_loss = criterion_CE(middle_output3, targets)
                middle3_losses.update(middle3_loss.item(), inputs.size(0))
